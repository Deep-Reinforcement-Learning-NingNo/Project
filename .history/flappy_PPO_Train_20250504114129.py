...
wandb.init(
    ...
    name=f"PPO-lr{learning_rate:.0e}-bs{batch_size}-clip{clip_epsilon}-ep{epochs}",
    config={
        "algorithm": "PPO",
        "n_observations": n_observations,
        "n_actions": num_of_action,
...
for episode in tqdm(range(n_episodes)):
    episode_reward, policy_loss, value_loss = agent.learn(
        env=env,
        max_steps=max_steps_per_episode, seed
    )
    ...
    wandb.log({
        ...
        "episode": episode   # logging the episode number
    })
    total_reward += episode_reward
    total_policy_loss += policy_loss
    total_value_loss += value_loss

    # Log & print every 100 episodes
    if (episode + 1) % 100 == 0:
        ...
        wandb.log({
            ...
            "episode": episode + 1  # logging the episode number
        })
