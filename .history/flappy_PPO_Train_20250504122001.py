...
if(lidar_flag == 1):
    save_path = f"./saved_models/ppo(lidar)-lr{learning_rate:.0e}-dis{discount_factor}-bs{batch_size}-clip{clip_epsilon}-ep{epochs}-n_eps{n_episodes}"
else:
    save_path = f"./saved_models/ppo(normal)-lr{learning_rate:.0e}-dis{discount_factor}-bs{batch_size}-clip{clip_epsilon}-ep{epochs}-n_eps{n_episodes}"
...
if(lidar_flag == 1):
    wandb_name = f"PPO(lidar)-lr{learning_rate:.0e}-dis{discount_factor}-bs{batch_size}-clip{clip_epsilon}-ep{epochs}-n_eps{n_episodes}"
else:
    wandb_name = f"PPO(normal)-lr{learning_rate:.0e}-dis{discount_factor}-bs{batch_size}-clip{clip_epsilon}-ep{epochs}-n_eps{n_episodes}"
# --- Init WandB ---
